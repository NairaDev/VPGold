...
if($action == 'addProduct'){
	...
	$add_product = $model->add_product($catId,$prodName,$prodPrice,$prodDesc,$image_name);
...
if($action == 'open_update_modal'){
	$prodId = $_POST['prodId'];
	$get_product_by_id = $model->get_product_by_id($prodId);
...
if($action == 'save_form_details'){
	...
	$save_edit_prod = $model->save_edit_prod($iProdId,$vProdName,$iProdPrice,$vProdDesc,$image_name, $vProdStatus);
...
if($action == 'delet_prod'){
	...
	$delete_prod = $model->delete_prod($iProdid);
...
if($action == 'confirmOrder'){
	...
	$confirmOrder = $model->confirmOrder($orderId);
	...
	if($confirmOrder){
		...
		$returnArr['Action'] = '0';
		$returnArr['message'] = 'Failed to confirn order';
	}
