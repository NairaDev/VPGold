 ...
 <div class="row">
	...
	<div class="row">
		<div class="col-4 offset-4">
			<form style="margin-top:20px;" action="../controller/productController.php" method="post" enctype="multipart/form-data">
	<div class = 'add_product_container text-center' >
		<div class="row">
			<div class="col-6">
				<div>
					<label for="prodName" class="text-left d-block">Name</label>
					<input type="text" name="prodName" class="prodName form-control">
					
				</div>
			</div>
			<div class="col-6">
			<div>
				<label for="prodPrice" class="text-left d-block">Price</label>
				<input type="text" name="prodPrice" class="prodPrice form-control">
			</div>
				
			</div>
		</div>
		<div>
					<label for="prodDesc" class="text-left d-block">Description</label>
					<textarea name="prodDesc" class="prodDesc form-control mb-3" row="4"></textarea>
				</div>
...
<button type="submit" class="btn btn-success" style="padding:10px 47px; margin-right:15px;" name="action" value="addProduct">Add</button>
...
<div class="alert alert-danger delete_danger" role="alert" style="display: none;"></div>
	...
	<div class="show_products_container">
		<?php 
		$display_products = $model->display_products($catId);
		if(count($display_products)>0){ ?>
			<table class="table table-hover" style="cellpadding:20px" >
				<thead>
					<tr>
						<th style="  font-size:20px; color:#1E1E1E; text-align:center;">Image</th>
						<th style="  font-size:20px; color:#1E1E1E; text-align:center;">Name</th>
						<th style="  font-size:20px; color:#1E1E1E; text-align:center;">Price</th>
						<th style="  font-size:20px;  text-align:center;">Description</th>
						<th style="  font-size:20px;  text-align:center;">Status</th>
						<th style="  font-size:20px; text-align:center;">Actions</th>
					</tr>
				</thead>
				<tbody>
					<?php foreach($display_products as $product){ ?>
						<tr>
							...
							<td style=""><?=$product['vProdName']?></td>
							<td style=" "><?=$product['iProdPrice']." "?>AMD</td>
							<td style=" "><?=$product['vProdDesc']?></td>
							<td style=" "><?=$product['eStatus']?></td>
							<td>
								<div class="d-flex align-items-center">
								<button class="btn btn-primary updateProd" style="padding:10px 47px; margin-right:15px;" data-id="<?=$product['iProdId']?>">Update</button>
								<button class="btn btn-danger deleteProd"  style="padding:10px 50px;" data-id="<?=$product['iProdId']?>" >Delete</button>
							</div>
							</td>
						</tr>
					<?php } ?>
