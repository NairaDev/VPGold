        ...
        if(count($allNew)>0){ ?>
           <?php foreach($allNew as $new){ ?>
            ...
            <div class="card" style="width: 100%;">
                <img width="100%" height="300px" class="card-img-top" src="../../admin/assets/img/<?=$new['vProdImage']?>   " alt="Card image cap">
                <div class="card-body"  style="text-align:center">
                    <h5 class="card-title"><?=$new['vProdName']?></h5>
                    <h5 class="card-title">Price: <?=$new['iProdPrice']?> </h5>
                    <p class="card-text" style="font-weight:800;"> 20% discount.</p>
                    <p class="card-text" style="font-weight:800;"><?=substr($new['vProdDesc'],0,50).' ...';?></p>
                    <button class="btn btn-success but addToCard" style="padding: 10px 60px;" data-value='<?=$userEmail?>' value='<?=$new['iProdId']?>'>Add to Card</button>
                </div>
               </div>
            </div>
           <?php }
